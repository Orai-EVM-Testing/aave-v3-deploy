...
import { makeSuite, TestEnv } from "../utils/make-suite-sapphire";
...
makeSuite("Mainnet Check list", (testEnv: TestEnv) => {
  ...
  it("Borrow USDC", async () => {
    const { users, usdc, pool, helpersContract} = testEnv;
  ...
  it("Check reserves data", async () => {
    const { users, wrose, usdc, pool, helpersContract, uiPoolDataProvider, addressesProvider, walletBalanceProvider } =
      testEnv;
